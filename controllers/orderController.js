...
export const requestRefund = async (req, res) => {
  try {
    const { orderId, reason, items, message } = req.body;
    const images = req.files ? req.files.map(file => file.path) : [];
    const userId = req.user._id;

    const order = await Order.findById(orderId).populate({
      path: 'items.product',
      select: 'sellerId name price'
    });
    ...
    const sellerIds = [...new Set(order.items
      .filter(item => item.product && item.product.sellerId)
    ...
    for (const sellerId of sellerIds) {
      await NotificationService.create({
        ...
        message: `Order #${order._id} has requested a refund`,
        data: {
          orderId: order._id,
          status: 'pending',
          amount: refundAmount,
...
// Respond to refund request (seller/admin)
export const respondToRefundRequest = async (req, res) => {
  try {
    const { orderId, status, responseMessage } = req.body;
    const userId = req.user._id;
    ...
    if (!order || !order.refundRequest) {
      return res.status(404).json({
        success: false,
        message: "Order or refund request not found"
      });
    }

    // Prevent multiple responses to same refund request
    if (order.refundRequest.status !== 'pending') {
      return res.status(400).json({
        success: false,
        message: `Refund request has already been ${order.refundRequest.status}`
      });
    }
    ...
    if (!isAdmin) {
      const hasSellerProducts = order.items.some(item => 
        item.product && item.product.sellerId.toString() === userId.toString()
      );

      if (!hasSellerProducts) {
        return res.status(403).json({
          success: false,
          message: "You don't have permission to respond to this refund request"
        });
      }
    }
    ...
    if (status === 'approved') {
      try {
        ...
        order.refundInfo = {
          ...
          items: order.refundRequest.items
        };
        ...
        order.tracking.history.push({
          status: 'Refunded',
          timestamp: new Date(),
          note: `Refund approved: ${responseMessage}`,
          updatedBy: userId
        });
      } catch (error) {
      ...
      // For rejected refunds, update order tracking
      if (!order.tracking) order.tracking = { history: [] };
      order.tracking.history.push({
        status: 'Refund Rejected',
        timestamp: new Date(),
        note: `Refund rejected: ${responseMessage}`,
        updatedBy: userId
      });
    }
    ...
    order.refundRequest.responseMessage = responseMessage;
    order.refundRequest.respondedAt = new Date();
    order.refundRequest.respondedBy = userId;
    ...
    await NotificationService.create({
      ...
      title: `Refund Request ${status === 'approved' ? 'Approved' : 'Rejected'}`,
      message: `Your refund request for order #${order._id} has been ${status}`,
      data: {
        orderId: order._id,
        status,
        responseMessage,
    ...
    return res.status(500).json({
      success: false,
      message: "Internal server error while processing refund response"
    });
  }
};
...
export const processRefund = async (req, res) => {
  try {
    const { orderId, reason, refundItems, refundAmount } = req.body;
    const userId = req.user._id;
    const isAdmin = req.user.role.includes("admin");
    ...
    if (!isAdmin) {
      ...
      if (!hasSellerProducts) {
        return res.status(403).json({
          success: false,
          message:
            "You don't have permission to process a refund for this order",
        });
      }
    }
    ...
    if (
      !["Confirmed", "Pending Confirmation", "Shipped"].includes(order.status)
    ) {
      return res.status(400).json({
        success: false,
        message: `Cannot process refund for order in ${order.status} status`,
      });
    }
    ...
    await NotificationService.create({
      ...
      message: `Your order #${order._id} has been refunded. Amount: ${
    ...
    // If admin processed the refund, notify seller
    if (isAdmin) {
      const sellerIds = [
        ...new Set(
          order.items
      ...
      for (const sellerId of sellerIds) {
        await NotificationService.create({
          ...
          message: `Order #${order._id} has been refunded by an admin.`,
          data: {
            orderId: order._id,
            status: "Refunded",
            refundAmount: refundAmount || order.amount,
...
export const updateOrderStatus = async (req, res) => {
  try {
    ...
    // Find and populate order with product details for notifications
    const order = await Order.findById(orderId).populate({
      path: "items.product",
      select: "sellerId name price",
    });
    ...
    order.items.forEach((item) => {
      if (item.product && item.product.sellerId) {
        const sellerId = item.product.sellerId.toString();
        if (!sellerItems[sellerId]) {
          sellerItems[sellerId] = [];
        }
        sellerItems[sellerId].push(item);
    ...
    // Notify each seller about their items' status update
    for (const [sellerId, items] of Object.entries(sellerItems)) {
      // Don't notify the seller who made the update
    ...
    // If admin made the update, notify all sellers
    if (isAdmin) {
      for (const [sellerId, items] of Object.entries(sellerItems)) {
        await NotificationService.create({
          recipient: sellerId,
          ...
          message: `Admin has updated order #${orderId} status to ${status}`,
          data: {
            orderId: order._id,
            status: status,
            items: items,
